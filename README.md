# Portofor 🚀 CLI

<p align="left">
  <a href="https://github.com/aldifayol/portfofor.my.id/stargazers" target="_blank">
    <img src="https://img.shields.io/github/stars/aldifayol/portofor.my.id?style=social" alt="GitHub stars" />
  </a>
  <a href="https://github.com/aldifayol/portofor.my.id" target="_blank">
    <img src="https://komarev.com/ghpvc/?username=aldifayol&label=Views&color=blue&style=flat" alt="Profile views" />
  </a>
  <a href="https://www.npmjs.com/package/portofor" target="_blank">
    <img src="https://img.shields.io/npm/v/portofor?color=cb3837&logo=npm" alt="NPM version" />
  </a>
  <a href="https://www.npmjs.com/package/portofor" target="_blank">
    <img src="https://img.shields.io/npm/dt/portofor?color=cb3837&logo=npm" alt="NPM downloads" />
  </a>
</p>
<br/>

![Build the web you want](.github/assets/banner.png 'Build the web you want')

<p align="center">
  <br/>
  <a href="https://portofor.my.id">Portofor</a> is a modern dark minimalist narcissistic portfolio website with blogging functionality built with Astro&mdash;
  <br/>
  powerful developer experience meets lightweight output.
  <br/><br/>
</p>

## Installation

The **recommended** way to install the latest version of Astro is by running the command below:

```bash
npm create portofor@latest
```

Looking for help? Start with our [Getting Started](https://portofor.my.id/#getting-started/) guide.

Looking for quick examples? [Open a starter project](https://starter.portofor.my.id/) right in your browser.

## Reference & Inspiration

**See what you can build [here](https://porto-blog-astro.edgeone.dev)**

<p align="center">
  <img src=".github/assets/ref.webp" alt="Demo Screenshot" width="700" />
</p>

## Documentation

Visit our [official documentation](https://portofor.my.id/docs).

## Support

Having trouble? Get help in the official [Portofor Discord](https://discord.com/portofor).

## Contributing

**New contributors welcome!** Check out our [Contributors Guide](CONTRIBUTING.md) for help getting started.

## Links

- [License (MIT)](LICENSE)
- [Buy Me A Coffee](https://saweria.co/portofor)
- [Website](https://portofor.my.id)

## Sponsors

Astro is free, open source software made possible by these wonderful sponsors.

[❤️ Sponsor Portofor! ❤️](https://github.com/aldifayol/.github/blob/main/FUNDING.md)

<p align="left">
  <a target="_blank" href="https://opencollective.com/portofor">
    <img src="https://portofor.my.id/sponsors.png" alt="Portofor Sponsors" />
  </a>
</p>
